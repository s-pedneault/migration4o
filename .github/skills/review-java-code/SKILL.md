...
1. **Avoid redundant code**: Eliminate duplicate code and unnecessary complexity. Ensure that the code is efficient and maintainable. If we have redundant code, we need to move it to a utility class to have ONE source of truth. DO not create unnecessary method overloads, improve existing methods when possible and invoke them directly.
...
1. **Prefer moving numerous arguments to a context or attributes class**: When a method has many parameters, consider encapsulating them in a dedicated context or attributes class to improve readability and maintainability.
