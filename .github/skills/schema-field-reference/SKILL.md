...
|---|---|
| Explicit target class name | `field.pointsTo` — fully-qualified FQN, or null |

## Resolving the target entity class

```java
// 1. Prefer explicit override
String targetFQN = field.pointsTo;

// 2. Fall back to type-name convention when not set
if (targetFQN == null) {
    targetFQN = TypeUtil.resolveIDEntiteTarget(field.type, schema);
}

// 3. Resolve to schema class
DOSchemaClass targetClass = SchemaUtil.findClassByName(targetFQN, schema);
if (targetClass != null && field.embedContents) {
    // inline all fields of the target entity
}
```

## Notes
- `field.pointsTo` is set only when the target cannot be inferred from `field.type` by naming convention.
- Distinct from `DOSchemaClass.pointsTo` which serves the same role at the class level.
- When `field.embedContents == false`, only the numeric mID is written to XML and `pointsTo` is irrelevant for export.
